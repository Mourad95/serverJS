import { Registry, Type } from "@tsed/core";
import { JSONSchema6 } from "json-schema";
import { JsonSchema } from "../class/JsonSchema";
export declare class JsonSchemaRegistry extends Registry<any, Partial<JsonSchema>> {
    /**
     *
     * @param {Type<any>} target
     * @param {string} propertyKey
     * @param type
     * @param collectionType
     * @returns {JsonSchema}
     */
    property(target: Type<any>, propertyKey: string, type: any, collectionType?: any): JsonSchema;
    /**
     *
     * @param schema
     * @param type
     * @param collectionType
     */
    private static createJsonSchema(schema, type, collectionType?);
    /**
     *
     * @param target
     * @param {string} propertyKey
     * @param value
     */
    required(target: any, propertyKey: string, value?: boolean): any;
    /**
     *
     * @param {Type<any>} target
     * @returns {JSONSchema6}
     */
    getSchemaDefinition(target: Type<any>): JSONSchema6;
    /**
     *
     * @param {Type<any>} target
     * @returns {JSONSchema6}
     */
    private getSchema(target);
    /**
     *
     * @param {JsonSchema} schema
     * @param definitions
     */
    private findReferences(schema, definitions);
    /**
     *
     * @param {JSONSchema4} schema
     * @param definitions
     * @returns {JSONSchema4}
     */
    private getRef(schema, definitions);
    /**
     *
     * @param {string} ref
     * @returns {string}
     */
    private getRefName(ref);
    /**
     *
     * @param {string} name
     * @returns {JsonSchema}
     */
    private getSchemaByName(name);
}
export declare const JsonSchemesRegistry: JsonSchemaRegistry;
