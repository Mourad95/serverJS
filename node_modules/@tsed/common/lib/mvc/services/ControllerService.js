...
let ControllerService = class ControllerService extends core_1.ProxyMap {
    ...
    constructor(injectorService, expressApplication, settings, routeService) {
        super(injectorService, { filter: { type: ProviderType_1.ProviderType.CONTROLLER } });
    ...
    $onRoutesInit(components) {
        ts_log_debug_1.$log.info("Map controllers");
    ...
    buildRouters() {
        ...
        this.forEach((provider) => {
            if (!provider.router && !provider.hasParent()) {
                new ControllerBuilder_1.ControllerBuilder(provider, defaultRoutersOptions).build(this.injectorService);
            }
        });
    }
    /**
...
ControllerService = tslib_1.__decorate([
    service_1.Service(),
    tslib_1.__param(1, decorators_1.ExpressApplication),
